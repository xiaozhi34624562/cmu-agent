...
def parse_args():
    ...
    parser.add_argument('--info_seek_path')
    parser.add_argument('--infoseek_data_path',default='/home/share/linhaoqiang/MMIR/sft_data/infoseek_used_images2')
    parser.add_argument('--generated_data')
    parser.add_argument('--trained_passages_path')
    parser.add_argument('--arxiv_pages')
    parser.add_argument('--tat_pages')
    parser.add_argument('--trained_passages_path')
    # 训练参数
...
def contrastive_loss(query_fts, pos_fts, neg_fts, temperature=0.07):
    """
    计算多正例的对比损失函数，具体形式为：
    mean(Σ_每个正例 -log(e(这个正例) / (e(这个正例) + e(所有负例))))

    :param query_fts: 查询特征，大小为 [ft_dim]
    :param pos_fts: 正样本特征，大小为 [pos_num, ft_dim]
    :param neg_fts: 负样本特征，大小为 [neg_num, ft_dim]
    :param temperature: 温度缩放因子，默认为0.07
    :return: 对比损失值
    """
...
class M2RKDataset(Dataset):
    def __init__(self, data_path, preprocessor, split="val", mode="default"):
        ...
        with open(os.path.join("", args.info_seek_path), "r") as f:
            self.infoseek_contents = json.load(f)

        with open(os.path.join("", args.generated_data), "r") as f:
            self.queries = json.load(f)
        M2KR_passages_path = os.path.join("",args.trained_passages_path)
        ...
        self.infoseek_content_dict = {content["passage_id"]: content["passage_content"] for content in self.infoseek_contents}
    ...
    def __getitem__(self, idx):
        ...
        if type == "M2KR":
            # Query details
            query_id = query["qid"]
            instruction_question = query['query']

            # Retrieve positive and negative contents based on item IDs
            pos_contents = [
                f"The main content of the document is :{get_first_300_words(self.M2KR_content_dict[pid])}. Please describe the document."
                for pid in query["pos_item"]]
            neg_contents = [
                f"The main content of the document is :{get_first_300_words(self.M2KR_content_dict[pid])}. Please describe the document."
                for pid in query["neg_item"]]

            # Load image
            #img_path = os.path.join("/home/share/linhaoqiang/MMIR/M2KR/images", f"{query['img_id']}.jpg")
            img_path = find_image_path(query['img_id'])
            image = Image.open(img_path).convert("RGB")
            image_resized = image.resize((384, 384))
            instruction_question = f"Given the reference image and with the instruction: {instruction_question}. Try to find the document with the reference image and the instruction. "

            return query_id, instruction_question, image_resized, pos_contents[0], neg_contents[0],type
        ...
        if type == "Infoseek":
            ...
            # Retrieve positive and negative contents based on item IDs
            pos_contents = [f"The main content of the document is :{get_first_300_words(self.infoseek_content_dict[pid])}. Please describe the document." for pid in query["pos_item"]]
            neg_contents = [f"The main content of the document is :{get_first_300_words(self.infoseek_content_dict[pid])}. Please describe the document." for pid in query["neg_item"]]

            # Load image
            img_path = os.path.join(self.data_path,  f"{query['img_id']}.jpg")
        ...
        #image = self.preprocessor(image)
            ...
            query_id = query["qid"]
            instruction_question = query["query"]
            pos_doc_img_path = f"{query['pos_item'][0]['doc_name']}_{query['pos_item'][0]['page_id']}.png"
            neg_doc_img_path = f"{query['neg_item'][0]['doc_name']}_{query['neg_item'][0]['page_id']}.png"
            pos_doc_img_path = re.sub(r'[<>:"/\\|?*]', '_', pos_doc_img_path)
            neg_doc_img_path = re.sub(r'[<>:"/\\|?*]', '_', neg_doc_img_path)
            if type == "Arxiv":
                pos_doc_img_path = os.path.join(args.arxiv_pages , pos_doc_img_path)
                neg_doc_img_path = os.path.join(args.arxiv_pages, neg_doc_img_path)
            else:
                pos_doc_img_path = os.path.join(args.tat_pages , pos_doc_img_path)
                neg_doc_img_path = os.path.join(args.tat_pages, neg_doc_img_path)

            pos_img = Image.open(pos_doc_img_path).convert('RGB')
            neg_img = Image.open(neg_doc_img_path).convert('RGB')
            return query_id, instruction_question, 0, pos_img, neg_img,type
...
training_args = TrainingArguments(
    ...
    gradient_accumulation_steps=64,  # 每累计128个小批次后更新一次梯度
...
class CustomTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs):
        ...
        if type == "Arxiv" or type == "Tat":
            ...
            # print(111)
            with torch.no_grad():
                target_neg_fts = model.get_fused_embeddings(images=neg_contents)
        else:
            ...
            #print(111)
            target_neg_fts = model.get_fused_embeddings(texts=neg_contents)
...
dataset = M2RKDataset(data_path= args.infoseek_data_path,preprocessor=None,split="train",mode="default")
#time.sleep(30)
...
python train_script.py \
  ...
  --output_dir ./results/gme_lora_full \
  --lora_r 64 \
  --lora_alpha 16 \
  --lora_dropout 0.05 \
  --learning_rate 1e-5 \
